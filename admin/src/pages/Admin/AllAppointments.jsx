  ...
  return (
    <div className='w-full max-w-6xl m-5'>
      ...
      <div className='bg-white border rounded text-sm max-h-[80vh] min-h-[60vh] overflow-y-scroll'>
        <div className='hidden sm:grid grid-cols-[0.5fr_3fr_1fr_3fr_3fr_1fr_1fr] grid-floe-col
        py-3 px-6 '>
          <p>#</p>
          <p>Patient</p>
          <p>Age</p>
        ...
        {appointments.map((item,index)=>(
          ...
          items-center text-gray-500 py-3 px-6 border hover:bg-gray-50' key={index}>
           ...
           <p> {item.slotDate ? slotDateFormat(item.slotDate) : "No Date"}, {" "}
                                {item.slotTime || "No Time"}</p>

          <div className='flex items-center gap-2'>
              <img className='w-10 bg-gray-200 rounded-full' src={item.docData.image} alt="" /> <p>{item.docData.name}</p>
            </div>

            <p>INR {item.docData.fees}</p>
